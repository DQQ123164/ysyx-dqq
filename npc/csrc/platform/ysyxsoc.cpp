...
static const char *lookup_region(paddr_t address, const Region *map, size_t count) {
  for (size_t i = 0; i < count; i ++) {
    if (address_in_window(address, map[i].base, map[i].size)) return map[i].name;
  }
  return NULL;
}
...
bool mem_read(paddr_t address, int length, word_t *value) {
  ...
  if (!access_fits(address, length, NPC_SDRAM_BASE, NPC_SDRAM_SIZE)) return false;
...
#else
const char *mem_region(paddr_t address) { (void)address; return NULL; }
#endif
...
void difftest_copy_memory(void (*copy)(paddr_t, void *, size_t, bool), bool dir) {
  ...
  if (loaded_size > 0) copy(NPC_FLASH_BASE, flash_mem, (size_t)loaded_size, dir);
}
...
extern "C" void flash_read(int addr, int *out) {
  ...
  Assert(offset <= NPC_FLASH_SIZE - 4,
      "flash read out of bound image: addr=0x%08x pc=" FMT_WORD,
      (uint32_t)addr, cpu.pc);
  *out = (int)host_read(flash_mem + offset, 4);
}

extern "C" int sdram_read16(unsigned int index) {
  uint32_t offset = index * 2u;
  Assert(offset <= NPC_SDRAM_SIZE - 2,
      "[ERROR] sdram read out of bound memory: word=0x%08x pc=" FMT_WORD,
      index, cpu.pc);
  return (int)host_read(sdram_mem + offset, 2);
}

extern "C" void sdram_write16(
    unsigned int index, unsigned int value, unsigned int strb) {
  uint32_t offset = index * 2u;
  Assert(offset <= NPC_SDRAM_SIZE - 2,
      "[ERROR] sdram write out of bound memory: word=0x%08x pc=" FMT_WORD,
      index, cpu.pc);
  if (strb & 1u) sdram_mem[offset] = value & 0xffu;
  if (strb & 2u) sdram_mem[offset + 1] = (value >> 8) & 0xffu;
}
