...
static void validate_retirement_address(
    vaddr_t retired_pc, vaddr_t next_pc, uint32_t instruction) {
  Assert((retired_pc & 3u) == 0,
      "retirement address is unaligned: pc=" FMT_WORD " inst=0x%08x",
      retired_pc, instruction);
  Assert((next_pc & 3u) == 0,
      "successor address is unaligned: pc=" FMT_WORD " inst=0x%08x",
      next_pc, instruction);
}
...
static void validate_privileged_transition(
  ...
  if (instruction == INST_ECALL) {
    ...
    Assert(saved_pc && machine_trap && entered_vector
        && interrupts_off && machine_mode_saved,
...
static void install_arch_state(
    vaddr_t next_pc, word_t mstatus, word_t mtvec,
    word_t mepc, word_t mcause, const uint32_t *gpr) {
...
extern "C" void cpu_commit(
  ...
  Assert(!mailbox.occupied,
      "retirement queue overflow: queued=" FMT_WORD " incoming=" FMT_WORD,
      mailbox.pc, retired_pc);
  Assert(gpr != NULL, "retirement event has no register image at " FMT_WORD,
      retired_pc);

  validate_retirement_address(retired_pc, next_pc, retired_inst);
  validate_privileged_transition(retired_pc, retired_inst, next_pc,
...
bool cpu_take_commit(vaddr_t *pc, uint32_t *instruction) {
  Assert(pc != NULL && instruction != NULL,
      "retirement consumer received a null destination");
...
void cpu_reset(vaddr_t reset_pc) {
  Assert((reset_pc & 3u) == 0,
      "reset address is unaligned: " FMT_WORD, reset_pc);
